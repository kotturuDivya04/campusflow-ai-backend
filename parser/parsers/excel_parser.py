...
class ExcelParser(BaseParser):
    """Concrete parser implementation for reading Excel (.xlsx, .xls) files."""
    
    def parse(self, file_path: str) -> list[dict[str, Any]]:
        """Parses an Excel spreadsheet, automatically detecting the header row if leading rows exist."""
        logger.info(f"Starting Excel parsing for file: {file_path}")
        
        try:
            # Load the first sheet without assuming header position
            df_raw = pd.read_excel(file_path, header=None)
        except Exception as e:
            logger.error(f"Failed to read Excel file structure: {e}")
            raise ValueError(f"Invalid Excel format or file corrupted: {e}")
        ...
        try:
            ...
            logger.error(f"Failed to reload Excel file with header at row {header_idx}: {e}")
            raise ValueError(f"Error parsing Excel rows: {e}")
        ...
        logger.info(f"Extracted {len(records)} raw records from Excel.")
        return records
